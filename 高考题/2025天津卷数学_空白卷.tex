...
\subq{（1）}{求证：$GF\perp$ 平面 $FBE$；}

\subq{（2）}{求平面 $FBE$ 与平面 $EBG$ 夹角的余弦值；}

\subq{（3）}{求三棱锥 $D-FBE$ 的体积. }
\end{minipage}\hfill
...
\subq{（2）}{过点 $P$ 的直线与椭圆有唯一交点 $B$（异于点 $A$），求证：$PF$ 平分 $\angle AFB$. \newpage}
\item（15分）

已知数列 $\{a_n\}$ 是等差数列，$\{b_n\}$ 是等比数列，$a_1=b_1=2$，$a_2=b_2+1$，$a_3=b_3$. 

\subq{（1）}{求 $\{a_n\}$，$\{b_n\}$ 的通项公式；}

\subq{（2）}{$\forall n\in\mathbb N^*$，$I\in\{0,1\}$，有 $T_n=\{p_1a_1b_1+p_2a_2b_2+\cdots+p_{n-1}a_{n-1}b_{n-1}+p_na_nb_n\mid p_1,p_2,\ldots,p_{n-1},p_n\in I\}$，}

\subq{（i）}{求证：对任意实数 $t\in T_n$，均有 $t<a_{n+1}b_{n+1}$；}

\subq{（ii）}{求 $T_n$ 所有元素之和. }

\item（16分）

已知函数 $f(x)=ax-(\ln x)^2$. 

\subq{（1）}{$a=1$ 时，求 $f(x)$ 在点 $(1,f(1))$ 处的切线方程；}

\subq{（2）}{$f(x)$ 有 $3$ 个零点，$x_1,x_2,x_3$ 且 $x_1<x_2<x_3$. }

\subq{（i）}{求 $a$ 的取值范围；}

\subq{（ii）}{证明 $(\ln x_2-\ln x_1)\cdot\ln x_3<\dfrac{4e}{e-1}$. }
\end{examanswers}
