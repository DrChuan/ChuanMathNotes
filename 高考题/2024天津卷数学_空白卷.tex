...
\begin{tightcenter}\includegraphics[width=3.0cm]{figures/2024_tj_q14.png}\end{tightcenter}
...
（2）过点 $\left(0,-\dfrac32\right)$ 的动直线与椭圆有两个交点 $P,Q$。在 $y$ 轴上是否存在点 $T$ 使得 $\overrightarrow{TP}\cdot\overrightarrow{TQ}\leq0$ 恒成立？若存在，求出这个 $T$ 点纵坐标的取值范围；若不存在，请说明理由。
...
（2）设 $b_n=\linsys{k,n=a_k,\\b_{n-1}+2k,a_k<n<a_{k+1}}$，$b_1=1$，其中 $k$ 是大于 $1$ 的正整数。

（i）当 $n=a_{k+1}$ 时，求证：$b_{n-1}\geq a_k\cdot b_n$；

（ii）求 $\displaystyle\sum_{i=1}^{S_n}b_i$。
...
（2）若 $f(x)\geq a(x-\sqrt x)$ 在 $x\in(0,+\infty)$ 时恒成立，求 $a$ 取值范围；

（3）若 $x_1,x_2\in(0,1)$，证明 $|f(x_1)-f(x_2)|\leq |x_1-x_2|^{\frac12}$。
