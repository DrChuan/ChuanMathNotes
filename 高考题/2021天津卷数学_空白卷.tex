...
\graphchoices{\includegraphics[width=4.0cm]{figures/2021_tj_q3_A.png}}{\includegraphics[width=4.0cm]{figures/2021_tj_q3_B.png}}{\includegraphics[width=4.0cm]{figures/2021_tj_q3_C.png}}{\includegraphics[width=4.0cm]{figures/2021_tj_q3_D.png}}
\item 从某网格平台推荐的影视作品中抽取 $400$ 部，统计其评分分数据，将所得 $400$ 个评分数据分为 $8$ 组：$[66,70),[70,74),\ldots,[94,98]$，并整理得到如下的频率分布直方图，则评分在区间 $[82,86)$ 内的影视作品数量是
\begin{tightcenter}\includegraphics[width=8.0cm]{figures/2021_tj_q4.png}\end{tightcenter}
...
\twochoices{\tallchoice{$\left(2,\dfrac94\right]\cup\left[\dfrac52,\dfrac{11}4\right)$}}{$\left(\dfrac74,2\right)\cup\left(\dfrac52,\dfrac{11}4\right)$}{\tallchoice{$\left(2,\dfrac94\right]\cup\left[\dfrac{11}4,3\right)$}}{$\left(\dfrac74,2\right)\cup\left[\dfrac{11}4,3\right)$}
...
\subq{（1）}{求 $a$ 的值；}
\subq{（2）}{求 $\cos C$ 的值；}
\subq{（3）}{求 $\sin\left(2C-\dfrac\pi6\right)$ 的值. }
...
\subq{（1）}{求证：$D_1F\parallel$ 平面 $A_1EC_1$；}

\subq{（2）}{求直线 $AC_1$ 与平面 $A_1EC_1$ 所成角的正弦值；}

\subq{（3）}{求二面角 $A-AC_1-E$ 的正弦值. }
\end{minipage}\hfill
\begin{minipage}[t]{0.30\linewidth}\vspace{-1.0em}\centering\includegraphics[width=\linewidth]{figures/2021_tj_q17.png}\end{minipage}
\newpage
\item（15分）
...
\subq{（2）}{直线 $l$ 与椭圆有唯一的公共点 $M$，与 $y$ 轴的正半轴交于点 $N$，过 $N$ 与 $BF$ 垂直的直线交 $x$ 轴于点 $P$. 若 $MP\parallel BF$，求直线 $l$ 的方程. }
...
\subq{（2）}{记 $c_n=b_{2n}+\dfrac1{b_n}$，$n\in\mathbb N^*$. }

\subq{（i）}{证明 $\{c_n^2-c_{2n}\}$ 是等比数列；}

\subq{（ii）}{证明 $\displaystyle\sum_{k=1}^n\sqrt{\dfrac{a_ka_{k+1}}{c_k^2-c_{2k}}}<2\sqrt2$. }

\item（16分）

已知 $a>0$，函数 $f(x)=ax-xe^x$. 

\subq{（1）}{求曲线 $y=f(x)$ 在点 $(0,f(0))$ 处的切线方程；}

\subq{（2）}{证明 $f(x)$ 存在唯一的极值点；}

\subq{（3）}{若存在 $a$，使得 $f(x)\leq a+b$ 对任意 $x\in\mathbb R$ 成立，求实数 $b$ 的取值范围. }
\end{examanswers}
