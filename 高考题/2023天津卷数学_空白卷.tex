...
\begin{tightcenter}\includegraphics[width=5.5cm]{figures/2023_tj_q4.png}\end{tightcenter}
...
\begin{tightcenter}\includegraphics[width=7.6cm]{figures/2023_tj_q7.png}\end{tightcenter}
...
\subq{（1）}{求证：$A_1N\parallel$ 平面 $C_1MA$；}

\subq{（2）}{求平面 $C_1MA$ 与平面 $ACC_1A_1$ 所成夹角余弦值；}

\subq{（3）}{求点 $C$ 到平面 $C_1MA$ 的距离. }
\end{minipage}\hfill
...
\subq{（2）}{已知点 $P$ 是椭圆上一动点（不与端点重合），直线 $A_2P$ 交 $y$ 轴于点 $Q$，若三角形 $A_1PQ$ 的面积是三角形 $A_2FP$ 面积的二倍，求直线 $A_2P$ 的方程. }

\item（15分）

已知 $\{a_n\}$ 是等差数列，$a_2+a_5=16$，$a_5-a_3=4$. 

\subq{（1）}{求 $\{a_n\}$ 的通项公式和 $\displaystyle\sum_{i=2^{n-1}}^{2^n-1}a_i$. }

\subq{（2）}{已知 $\{b_n\}$ 为等比数列，对于任意 $k\in\mathbb N^*$，若 $2^{k-1}\leq n\leq2^k-1$，则 $b_k<a_n<b_{k+1}$，}

\subq{（1）}{当 $k\geq2$ 时，求证：$2^k-1<b_k<2^k+1$；}

\subq{（2）}{求 $\{b_n\}$ 的通项公式及其前 $n$ 项和. }

\item（16分）

已知函数 $f(x)=\left(\dfrac1x+\dfrac12\right)\ln(x+1)$. 

\subq{（1）}{求曲线 $y=f(x)$ 在 $x=2$ 处切线的斜率；}

\subq{（2）}{当 $x>0$ 时，证明：$f(x)>1$；}

\subq{（3）}{证明：$\dfrac56<\ln(n!)-\left(n+\dfrac12\right)\ln n+n\leq1$. }
\end{examanswers}
