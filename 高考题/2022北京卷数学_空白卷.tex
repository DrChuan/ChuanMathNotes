...
\begin{tightcenter}\includegraphics[width=5.0cm]{figures/2022_bj_q7.png}\end{tightcenter}
...
\subq{（2）}{若 $b=6$，且 $\triangle ABC$ 的面积为 $6\sqrt3$，求 $\triangle ABC$ 的周长.}
\item（13分）
...
\subq{（1）}{求证：$MN\parallel$ 平面 $BCC_1B_1$；}

\subq{（2）}{再从条件①、条件②这两个条件中选择一个作为已知，求直线 $AB$ 与平面 $BMN$ 所成角的正弦值.
...
\vspace{-1em}\centering\includegraphics[width=\linewidth]{figures/2022_bj_q17.png}
\end{minipage}
\item（14分）
...
\subq{（1）}{估计甲在校运动会铅球比赛中获得优秀奖的概率；}

\subq{（2）}{设 $X$ 是甲、乙、丙在校运动会铅球比赛中获得优秀奖的总人数，估计 $X$ 的数学期望 $E(X)$；}

\subq{（3）}{在校运动会铅球比赛中，甲、乙、丙谁获得冠军的概率估计值最大？（结论不要求证明）}
\item（15分）
...
\subq{（2）}{过点 $P(-2,1)$ 作斜率为 $k$ 的直线与椭圆 $E$ 交于不同的两点 $B,C$，直线 $AB,AC$ 分别与 $x$ 轴交于点 $M,N$，当 $|MN|=2$ 时，求 $k$ 的值.}
\item（15分）
...
\subq{（2）}{设 $g(x)=f'(x)$，讨论函数 $g(x)$ 在 $[0,+\infty)$ 上的单调性；}

\subq{（3）}{证明：对任意的 $s,t\in(0,+\infty)$，有 $f(s+t)>f(s)+f(t)$.}
\item（15分）
...
\subq{（1）}{判断 $Q:2,1,4$ 是否为 $5$-连续可表数列？是否为 $6$-连续可表数列？说明理由；}

\subq{（2）}{若 $Q:a_1,a_2,\cdots,a_k$ 为 $8$-连续可表数列，求证：$k$ 的最小值为 $4$；}

\subq{（3）}{若 $Q:a_1,a_2,\cdots,a_k$ 为 $20$-连续可表数列，且 $a_1+a_2+\cdots+a_k<20$，求证：$k\geq7$.}
\end{examanswers}
