...
（2）斜率为 $-\sqrt3$ 的直线与圆 $x^2+y^2=b^2$ 相切，且该直线交椭圆于 $P(x_1,y_1)$，$Q(x_2,y_2)(y_1<y_2)$，$A$ 是椭圆的上顶点. 记 $AP,AQ$ 的斜率分别为 $k_1,k_2$，求 $\dfrac{k_1}{k_2}$ 的值.
...
（2）记 $E_n=\{x\in\mathbb R\mid x\leq n,\exists k\in\mathbb N^*\text{ 使得 }x=a_k\text{ 或 }x=b_k\}$，$c_n$ 为 $E_n$ 中的元素个数.

（i）求 $c_{3^n}$；

（ii）求 $\displaystyle\sum_{m=1}^{3^n-1}(-1)^m\cdot a_m\cdot c_m$.
...
（2）当 $x\in\left[-\dfrac13,+\infty\right)$ 时，证明：$f(x)\geq1+\dfrac{x}{3}$；

（3）$\forall n\in\mathbb N^*$，$f(1)\cdot f\left(\dfrac12\right)\cdot f\left(\dfrac13\right)\cdots f\left(\dfrac1n\right)\geq(n+1)^a$ 恒成立，求实数 $a$ 的最大值.
\end{examanswers}
