...
\graphchoices{\includegraphics[width=5cm]{figures/2020_tj_q3_A.png}}{\includegraphics[width=5cm]{figures/2020_tj_q3_B.png}}{\includegraphics[width=5cm]{figures/2020_tj_q3_C.png}}{\includegraphics[width=5cm]{figures/2020_tj_q3_D.png}}
\newpage
...
\begin{tightcenter}\includegraphics[width=8cm]{figures/2020_tj_q4.png}\end{tightcenter}
...
\item 已知函数 $f(x)=\sin\left(x+\dfrac\pi3\right)$。给出下列结论：① $f(x)$ 的最小正周期为 $2\pi$；② $f\left(\dfrac\pi2\right)$ 是 $f(x)$ 的最大值；③把函数 $y=\sin x$ 的图象上所有点向左平移 $\dfrac\pi3$ 个单位长度，可得到函数 $y=f(x)$ 的图象。其中所有正确结论的序号是
\fourchoices{①}{①③}{②③}{①②③}
\item 已知函数 $f(x)=\linsys{x^3,x\geq0,\\-x,x<0}$。若函数 $g(x)=f(x)-|kx^2-2x|$（$k\in\mathbb R$）恰有 $4$ 个零点，则 $k$ 的取值范围是
\twochoices{$\left(-\infty,-\dfrac12\right)\cup(2\sqrt2,+\infty)$}{$\left(-\infty,-\dfrac12\right)\cup(0,2\sqrt2)$}{$(-\infty,0)\cup(0,2\sqrt2)$}{$(-\infty,0)\cup(2\sqrt2,+\infty)$}
...
\end{minipage}\hfill
\begin{minipage}[t]{0.3\linewidth}\vspace{0em}\centering\includegraphics[width=\linewidth]{figures/2020_tj_q15.png}\end{minipage}
...
（III）求 $\sin\left(2A+\dfrac\pi4\right)$ 的值。
...
（II）求二面角 $B-B_1E-D$ 的正弦值；

（III）求直线 $AB$ 与平面 $DB_1E$ 所成角的正弦值。
\end{minipage}\hfill
\begin{minipage}[t]{0.24\linewidth}\vspace{-1.4em}\centering\includegraphics[width=\linewidth]{figures/2020_tj_q17.png}\end{minipage}
...
（II）已知点 $C$ 满足 $3\overrightarrow{OC}=\overrightarrow{OF}$，点 $B$ 在椭圆上（$B$ 异于椭圆的顶点），直线 $AB$ 与以 $C$ 为圆心的圆相切于点 $P$，且 $P$ 为线段 $AB$ 的中点，求直线 $AB$ 的方程。
...
（II）记 $\{a_n\}$ 的前 $n$ 项和为 $S_n$，求证：$S_nS_{n+2}<S_{n+1}^2(n\in\mathbb N^*)$；

（III）对任意正整数 $n$，设 $c_n=\linsys{\dfrac{(3a_n-2)b_n}{a_na_{n+2}},\quad n\text{为奇数},\\\dfrac{a_{n-1}}{b_{n+1}},\quad n\text{为偶数}}$，求 $\{c_n\}$ 的前 $2n$ 项和。
...
（ii）求函数 $g(x)=f(x)-f'(x)+\dfrac9x$ 的单调区间和极值；

（II）当 $k>-3$ 时，求证：对任意的 $x_1,x_2\in[1,+\infty)$，且 $x_1>x_2$，有 $\dfrac{f'(x_1)+f'(x_2)}2>\dfrac{f(x_1)-f(x_2)}{x_1-x_2}$。
