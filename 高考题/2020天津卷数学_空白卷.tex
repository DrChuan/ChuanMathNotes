...
（3）求 $\sin\left(2A+\dfrac\pi4\right)$ 的值。
...
（2）求二面角 $B-B_1E-D$ 的正弦值；

（3）求直线 $AB$ 与平面 $DB_1E$ 所成角的正弦值。
\end{minipage}\hfill
\begin{minipage}[t]{0.27\linewidth}\vspace{-1em}\centering\includegraphics[width=\linewidth]{figures/2020_tj_q17.png}\end{minipage}
...
（2）记 $\{a_n\}$ 的前 $n$ 项和为 $S_n$，求证：$S_nS_{n+2}<S_{n+1}^2(n\in\mathbb N^*)$；

（3）对任意正整数 $n$，设 $c_n=\linsys{\dfrac{(3a_n-2)b_n}{a_na_{n+2}},\quad n\text{为奇数},\\\dfrac{a_{n-1}}{b_{n+1}},\quad n\text{为偶数}}$，求 $\{c_n\}$ 的前 $2n$ 项和。
...
（2）当 $k>-3$ 时，求证：对任意的 $x_1,x_2\in[1,+\infty)$，且 $x_1>x_2$，有 $\dfrac{f'(x_1)+f'(x_2)}2>\dfrac{f(x_1)-f(x_2)}{x_1-x_2}$。
