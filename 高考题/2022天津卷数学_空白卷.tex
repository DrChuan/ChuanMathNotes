...
\graphchoices{\includegraphics[width=3.6cm]{figures/2022_tj_q3_A.png}}{\includegraphics[width=3.6cm]{figures/2022_tj_q3_B.png}}{\includegraphics[width=3.6cm]{figures/2022_tj_q3_C.png}}{\includegraphics[width=3.6cm]{figures/2022_tj_q3_D.png}}
\newpage
...
\begin{tightcenter}\includegraphics[width=5.5cm]{figures/2022_tj_q4.png}\end{tightcenter}
...
\begin{tightcenter}\includegraphics[width=8.0cm]{figures/2022_tj_q8.png}\end{tightcenter}
\fourchoices{$23$}{$24$}{$26$}{$27$}
\item 已知 $f(x)=\dfrac12\sin2x$，关于该函数有下列四个说法：

① $f(x)$ 的最小正周期为 $2\pi$；

② $f(x)$ 在 $\left[-\dfrac\pi4,\dfrac\pi4\right]$ 上单调递增；

③当 $x\in\left[-\dfrac\pi6,\dfrac\pi3\right]$ 时，$f(x)$ 的取值范围为 $\left[-\dfrac{\sqrt3}{4},\dfrac{\sqrt3}{4}\right]$；

④ $f(x)$ 的图象可由 $g(x)=\dfrac12\sin\left(2x+\dfrac\pi4\right)$ 的图象向左平移 $\dfrac\pi8$ 个单位长度得到. 
...
\subq{（1）}{求证：$EF\parallel$ 平面 $ABC$；}

\subq{（2）}{求直线 $BE$ 与平面 $CC_1D$ 所成角的正弦值；}

\subq{（3）}{求平面 $A_1CD$ 与平面 $CC_1D$ 所成二面角余弦值. }
\end{minipage}\hfill
...
\subq{（1）}{求 $\{a_n\}$ 与 $\{b_n\}$ 的通项公式；}

\subq{（2）}{设 $\{a_n\}$ 的前 $n$ 项和为 $S_n$，求证：$(S_{n+1}+a_{n+1})b_n=S_{n+1}b_{n+1}-S_nb_n$；}

\subq{（3）}{求 $\displaystyle\sum_{k=1}^{2n}[a_{k+1}-(-1)^ka_k]b_k$. }
...
\subq{（2）}{直线 $l$ 与椭圆有唯一公共点 $M$，与 $y$ 轴相交于 $N$（$N$ 异于 $M$）. 记 $O$ 为坐标原点，若 $|OM|=|ON|$，且 $\triangle OMN$ 的面积为 $\sqrt3$，求椭圆的标准方程. }

\item（16分）

已知 $a,b\in\mathbb R$，函数 $f(x)=e^x-a\sin x$，$g(x)=b\sqrt x$. 

\subq{（1）}{求函数 $y=f(x)$ 在 $(0,f(0))$ 处的切线方程；}

\subq{（2）}{若 $y=f(x)$ 和 $y=g(x)$ 有公共点，}

\subq{（i）}{当 $a=0$ 时，求 $b$ 的取值范围；}

\subq{（ii）}{求证：$a^2+b^2>e$. }
\end{examanswers}
