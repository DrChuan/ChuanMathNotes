...
\begin{tightcenter}\includegraphics[width=10.0cm]{figures/2022_xgk2_q3.png}\end{tightcenter}
...
\end{minipage}\hfill
\begin{minipage}[t]{0.23\linewidth}\vspace{-1.6em}\centering\includegraphics[width=\linewidth]{figures/2022_xgk2_q11.png}\end{minipage}
...
\quad \quad 在某地区进行流行病学调查，随机调查了100位某种疾病患者的年龄，得到如下的样本数据的频率分布直方图：
\end{minipage}\hfill
\begin{minipage}[t]{0.44\linewidth}\vspace{-2.7em}\centering\includegraphics[width=\linewidth]{figures/2022_xgk2_q19.png}\end{minipage}
\subq{（1）}{估计该地区这种疾病患者平均年龄（同一组中的数据用该组区间的中点值为代表）；}

\subq{（2）}{估计该地区一位这种疾病患者的年龄位于区间 $[20,70)$ 的概率；}

\subq{（3）}{已知该地区这种疾病的患病率为 $0.1\%$，该地区年龄位于区间 $[40,50)$ 的人口占该地区总人口的 $16\%$. 从该地区中任选一人，若此人的年龄位于区间 $[40,50)$，求此人患这种疾病的概率. （以样本数据中患者的年龄位于各区间的频率作为患者的年龄位于该区间的概率，精确到0.0001）}
\item（12分）

如图，$PO$ 是三棱锥 $P-ABC$ 的高，$PA=PB$，$AB\perp AC$，$E$ 是 $PB$ 的中点. 

\noindent\begin{minipage}[t]{0.62\linewidth}
\subq{（1）}{证明：$OE\parallel$ 平面 $PAC$；}

\subq{（2）}{若 $\angle ABO=\angle CBO=30^\circ$，$PO=3$，$PA=5$，求二面角 $C-AE-B$ 的正弦值.}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\vspace{-1.2em}\centering\includegraphics[width=\linewidth]{figures/2022_xgk2_q20.png}\end{minipage}
\vspace{-2.7em}
\item（12分）
...
\subq{（2）}{过 $F$ 的直线与 $C$ 的两条渐近线分别交于 $A,B$ 两点，点 $P(x_1,y_1),Q(x_2,y_2)$ 在 $C$ 上，且 $x_1>x_2>0,y_1>0$. 过 $P$ 且斜率为 $-\sqrt3$ 的直线与过 $Q$ 且斜率为 $\sqrt3$ 的直线交于点 $M$. 从下面①②③中选取两个作为条件，证明另外一个成立：
...
\subq{（2）}{当 $x>0$ 时，$f(x)<-1$，求 $a$ 的取值范围；}

\subq{（3）}{设 $n\in\mathbb N^*$，证明：$\dfrac1{\sqrt{1^2+1}}+\dfrac1{\sqrt{2^2+2}}+\cdots+\dfrac1{\sqrt{n^2+n}}>\ln(n+1)$.}
\end{examanswers}
