...
\subq{（2）}{若 $\triangle ABC$ 的面积为 $3+\sqrt3$，求 $c$.}
\item（15分）
...
\subq{（2）}{若过 $P$ 的直线 $l$ 交 $C$ 于另一点 $B$，且 $\triangle ABP$ 的面积为9，求 $l$ 的方程.}
\item（15分）
...
\subq{（1）}{若 $AD\perp PB$，证明：$AD\parallel$ 平面 $PBC$；}

\subq{（2）}{若 $AD\perp DC$，且二面角 $A-CP-D$ 的正弦值为 $\dfrac{\sqrt{42}}7$，求 $AD$.}
\end{minipage}\hfill
...
\subq{（1）}{若 $b=0$，且 $f'(x)\geq0$，求 $a$ 的最小值；}

\subq{（2）}{证明：曲线 $y=f(x)$ 是中心对称图形；}

\subq{（3）}{若 $f(x)>-2$ 当且仅当 $1<x<2$，求 $b$ 的取值范围.}
\item（17分）
...
\subq{（1）}{写出所有 $(i,j)$，$1\leq i<j\leq6$，使数列 $a_1,a_2,\cdots,a_6$ 是 $(i,j)$-可分数列；}

\subq{（2）}{当 $m\geq3$ 时，证明：数列 $a_1,a_2,\cdots,a_{4m+2}$ 是 $(2,13)$-可分数列；}

\subq{（3）}{从 $1,2,\cdots,4m+2$ 中一次任取两个数 $i$ 和 $j(i<j)$，记数列 $a_1,a_2,\cdots,a_{4m+2}$ 是 $(i,j)$-可分数列的概率为 $P_m$，证明：$P_m>\dfrac18$.}
\end{examanswers}
