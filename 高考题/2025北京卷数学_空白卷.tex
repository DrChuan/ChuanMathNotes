...
\vspace{0em}\centering\includegraphics[width=\linewidth]{figures/2025_bj_q14.png}
\end{minipage}
...
\subq{（2）}{再从条件①、条件②、条件③这三个条件中选择一个作为已知，使得 $\triangle ABC$ 存在，求 $BC$ 边上的高.
...
\noindent\begin{minipage}[t]{0.62\linewidth}
\subq{（1）}{若 $F,G$ 分别为 $PD,PE$ 的中点，求证：$FG\parallel$ 平面 $PAB$；}

\subq{（2）}{若 $PA\perp$ 平面 $ABCD$，$PA=AC$，求直线 $AB$ 与平面 $PCD$ 所成角的正弦值.}
\end{minipage}\hfill
\begin{minipage}[t]{0.30\linewidth}
\vspace{-1em}\centering\includegraphics[width=\linewidth]{figures/2025_bj_q17.png}
\end{minipage}
\item（14分）
...
\subq{（1）}{估计甲校高一年级学生该题选择正确的概率 $p$；}

\subq{（2）}{从甲、乙两校高一年级学生中各随机抽取 $1$ 名，设 $X$ 为这 $2$ 名学生中该题选择正确的人数，估计 $X=1$ 的概率及 $X$ 的数学期望；}

\subq{（3）}{假设：如果没有掌握该知识点，学生就从题目给出的四个选项中随机选择一个作为答案；如果掌握该知识点，甲校学生选择正确的概率为 $100\%$，乙校学生选择正确的概率为 $85\%$. 设甲、乙两校高一年级学生掌握该知识点的概率估计值分别为 $p_1,p_2$，判断 $p_1$ 与 $p_2$ 的大小（结论不要求证明）.}
\item（15分）
...
\subq{（2）}{当 $-1<a<0$ 时，证明：除切点 $A$ 外，曲线 $y=f(x)$ 在直线 $l_1$ 上方；}

\subq{（3）}{设过点 $A$ 的直线 $l_2$ 与直线 $l_1$ 垂直，$l_1,l_2$ 与 $x$ 轴交点的横坐标分别是 $x_1,x_2$，若 $a>0$，求 $\dfrac{2a-x_2-x_1}{x_2-x_1}$ 的取值范围.}
\item（15分）
...
\subq{（1）}{对于第 $1$ 项为 $(3,3)$ 的 K 列，写出它的第 $2$ 项；}

\subq{（2）}{设 $\Gamma$ 为 K 列，且 $\Gamma$ 中项 $(x_i,y_i)(i=1,2,\cdots,n)$ 满足：当 $i$ 为奇数时，$x_i\in\{1,2,7,8\}$；当 $i$ 为偶数时，$x_i\in\{3,4,5,6\}$. 判断 $(3,2),(4,4)$ 能否同时为 $\Gamma$ 中的项，并说明理由；}

\subq{（3）}{证明：由 $M$ 的全部元素组成的序列都不是 K 列.}
\end{examanswers}
