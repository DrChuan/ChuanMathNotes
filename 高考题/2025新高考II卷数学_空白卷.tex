...
\subq{（2）}{设函数 $g(x)=f(x)+f\left(x-\dfrac\pi6\right)$，求 $g(x)$ 的值域和单调区间.}
\newpage
\item（15分）
...
\subq{（2）}{过点 $(0,-2)$ 的直线 $l$ 与 $C$ 交于 $A,B$ 两点，$O$ 为坐标原点，若 $\triangle OAB$ 的面积为 $\sqrt2$，求 $|AB|$.}
\item（15分）
...
\subq{（1）}{证明：$A'B\parallel$ 平面 $CD'F$；}

\subq{（2）}{求面 $BCD'$ 与面 $EFD'A'$ 所成的二面角的正弦值.}
\end{minipage}\hfill
...
\subq{（1）}{证明：$f(x)$ 在区间 $(0,+\infty)$ 存在唯一的极值点和唯一的零点；}

\subq{（2）}{设 $x_1,x_2$ 分别为 $f(x)$ 在区间 $(0,+\infty)$ 的极值点和零点.}

\subq{（i）}{设函数 $g(t)=f(x_1+t)-f(x_1-t)$，证明：$g(t)$ 在区间 $(0,x_1)$ 单调递减；}

\subq{（ii）}{比较 $2x_1$ 与 $x_2$ 的大小，并证明你的结论.}
\item（17分）
...
\subq{（1）}{求 $p_3,p_4$（用 $p$ 表示）；}

\subq{（2）}{若 $\dfrac{p_4-p_3}{q_4-q_3}=4$，求 $p$；}

\subq{（3）}{证明：对任意正整数 $m$，$p_{2m+1}-q_{2m+1}<p_{2m}-q_{2m}<p_{2m+2}-q_{2m+2}$.}
\end{examanswers}
