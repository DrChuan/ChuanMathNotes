...
\begin{tightcenter}\includegraphics[width=3.4cm]{figures/2024_xgk2_q14.png}\end{tightcenter}
\end{examquestions}
...
\subq{（2）}{若 $a=2$，$\sqrt2b\sin C=c\sin2B$，求 $\triangle ABC$ 的周长.}
\item（15分）
...
\subq{（2）}{若 $f(x)$ 有极小值，且极小值小于0，求 $a$ 的取值范围.}
\item（15分）
...
\subq{（1）}{证明：$EF\perp PD$；}

\subq{（2）}{求面 $PCD$ 与面 $PBF$ 所成的二面角的正弦值.}
\end{minipage}\hfill
...
\subq{（1）}{若 $p=0.4$，$q=0.5$，甲参加第一阶段比赛，求甲、乙所在队比赛成绩不少于5分的概率.}

\subq{（2）}{假设 $0<p<q$，}

\subq{（i）}{为使得甲、乙所在队的比赛成绩为15分的概率最大，应该由谁参加第一阶段比赛？}

\subq{（ii）}{为使得甲、乙所在队的比赛成绩的数学期望最大，应该由谁参加第一阶段比赛？}
\item（17分）
...
\subq{（1）}{若 $k=\dfrac12$，求 $x_2,y_2$；}

\subq{（2）}{证明：数列 $\{x_n-y_n\}$ 是公比为 $\dfrac{1+k}{1-k}$ 的等比数列；}

\subq{（3）}{设 $S_n$ 为 $\triangle P_nP_{n+1}P_{n+2}$ 的面积，证明：对任意的正整数 $n$，$S_n=S_{n+1}$.}
\end{examanswers}
