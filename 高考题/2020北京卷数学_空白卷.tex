...
\begin{tightcenter}\includegraphics[width=5.3cm]{figures/2020_bj_q4.png}\end{tightcenter}
...
\begin{tightcenter}\includegraphics[width=9.5cm]{figures/2020_bj_q15.png}\end{tightcenter}
给出下列四个结论：
...
④甲企业在 $[t_1,t_2]$，$[t_2,t_3]$，$[t_3,t_4]$ 这三段时间中，在 $[t_2,t_3]$ 的污水治理能力最强. 
...
\subq{（1）}{求证：$BC_1\parallel$ 平面 $AD_1E$；}

\subq{（2）}{求直线 $AA_1$ 与平面 $AD_1E$ 所成角的正弦值.}
\end{minipage}\hfill
\begin{minipage}[t]{0.30\linewidth}
\vspace{-1.0em}\centering\includegraphics[width=\linewidth]{figures/2020_bj_q16.png}
\end{minipage}
\item（13分）
...
\subq{（2）}{$\sin C$ 和 $\triangle ABC$ 的面积.
条件①：$c=7$，$\cos A=-\dfrac17$；条件②：$\cos A=\dfrac18$，$\cos B=\dfrac9{16}$.
...
\subq{（1）}{分别估计该校男生支持方案一的概率、该校女生支持方案一的概率；}

\subq{（2）}{从该校全体男生中随机抽取 $2$ 人，全体女生中随机抽取 $1$ 人，估计这 $3$ 人中恰有 $2$ 人支持方案一的概率；}

\subq{（3）}{将该校学生支持方案二的概率估计值记为 $p_0$. 假设该校一年级有 $500$ 名男生和 $300$ 名女生，除一年级外其他年级学生支持方案二的概率估计值记为 $p_1$，试比较 $p_0$ 与 $p_1$ 的大小.（结论不要求证明）}
\vspace{-1.2em}
\item（15分）

已知函数 $f(x)=12-x^2$.

\subq{（1）}{求曲线 $y=f(x)$ 的斜率等于 $-2$ 的切线方程；}

\subq{（2）}{设曲线 $y=f(x)$ 在点 $(t,f(t))$ 处的切线与坐标轴围成的三角形的面积为 $S(t)$，求 $S(t)$ 的最小值.}
\vspace{-1.2em}
\item（15分）
...
\subq{（2）}{过点 $B(-4,0)$ 的直线 $l$ 交椭圆 $C$ 于点 $M$，$N$，直线 $MA$，$NA$ 分别交直线 $x=-4$ 于点 $P$，$Q$，求 $\dfrac{|PB|}{|BQ|}$ 的值.}
\vspace{-1.2em}
...
\subq{（1）}{若 $a_n=n(n=1,2,\cdots)$，判断数列 $\{a_n\}$ 是否满足性质①，说明理由；}

\subq{（2）}{若 $a_n=2^{n-1}(n=1,2,\cdots)$，判断数列 $\{a_n\}$ 是否同时满足性质①、②，说明理由；}

\subq{（3）}{若 $\{a_n\}$ 是递增数列，且同时满足性质①和性质②，证明：$\{a_n\}$ 为等比数列.}
\end{examanswers}
