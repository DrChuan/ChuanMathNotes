...
\subq{（2）}{$\hat p$ 为首次故障时间小于 $365$ 天的概率估计值.}

\subq{（i）}{求 $\hat p$；}

\subq{（ii）}{工厂向某用户销售 $100$ 件电子元件，$X$ 为这 $100$ 件产品首次出现故障小于 $365$ 天的件数，则 $X\sim B(100,\hat p)$，求 $E(X),D(X)$.}
...
\subq{（1）}{证明：$CD\perp AB$；}

\subq{（2）}{若 $DE=2,BE=1,AE=\sqrt2,CD=2\sqrt3$，求 $AD$ 与平面 $ABC$ 所成角的正弦值.}
\end{minipage}\hfill
...
\subq{（1）}{证明：$\triangle ABC$ 为钝角三角形；}

\subq{（2）}{若 $\triangle ABC$ 面积为 $\dfrac{\sqrt7}{4}$，求 $\triangle ABC$ 周长.}
\newpage
\item（17分）
...
\subq{（2）}{$O$ 为坐标原点，给定点 $G(t_0,0)(t_0\neq0)$；$A(x_0,y_0)(y_0\neq0)$ 在 $E$ 上，过点 $A$ 作 $y$ 轴的垂线，交于点 $B$，$AO$ 与 $GB$ 交于点 $P$. 当 $A$ 在 $E$ 上运动时，$P$ 的轨迹为 $M$.}

\subq{（i）}{求 $M$ 的方程；}

\subq{（ii）}{$M$ 是否有中心点？当 $t_0$ 为何值时，$M$ 有中心点？当 $M$ 有中心点时，平移 $M$ 到 $M'$，使 $O$ 为 $M'$ 的中心点，说明 $M'$ 为何形状？}
...
\subq{（2）}{当 $x>0$ 时，$f(x+m)-f(x)>m$，求 $m$ 的取值范围；}

\subq{（3）}{当 $x>0$ 时，$f(x+k)+f(k-x)>2f(k)$，求 $k$ 的最小值.}
\end{examanswers}
