...
3.
假设 $P(x)$ 有至少 $n+1$ 个复零点，不妨设为 $x_1, x_2, \dots, x_{n+1}$（其中 $x_i, x_j$ 可以相等

应用第 2 题的结论 $n$ 次可知存在 0 次多项式 $Q(x)$， s.t.
$$P(x) = (x-x_1)(x-x_2)\cdots(x-x_n)Q(x)$$

则 $x_{n+1}$ 是 $Q(x)$ 的零点，而 $Q(x)$ 为非零常数多项式，没有零点，矛盾！$\implies$ 假设不成立，$P(x)$ 至多有 $n$ 个复零点

\sepline

4.
\textcolor{magenta}{（韦达定理）}

由 2，
\begin{align*}
    P(x) &= a_n \prod_{j=1}^n (x-x_i)\\
    &= a_n \sum_{k=0}^n \left( \sum_{1 \le j_1 < \dots < j_{n-k} \le n} \prod_{j=1}^{n-k} (-x_{m_j}) \right) x^{n-k} \\
    &= a_n \sum_{k=0}^n \left( (-1)^{k} \sum_{1 \le j_1 < \dots < j_{n-k} \le n} \prod_{j=1}^{k} x_{m_j} \right) x^{n-k}
\end{align*}

$x^{n-k}$ 的系数为 $a_n (-1)^{k} \sum_{1 \le j_1 < \dots < j_{n-k} \le n} \prod_{j=1}^{k} x_{m_j}$
...
令 $H(x_1, \dots, x_n) = F(x_1, \dots, x_n) - S(\sigma_{n,1}, \sigma_{n,2}, \dots, \sigma_{n,n-1})$，易知 $\sigma_{n,i}(x_1, \dots, x_{n-1}, 0) = \sigma_{n-1,i}(x_1, \dots, x_{n-1})$，所以$ H(x_1, \dots, x_{n-1}, 0) = G(x_1, \dots, x_{n-1}) - S(\sigma_{n-1,1}, \dots, \sigma_{n-1,n-1}) = 0$. 
